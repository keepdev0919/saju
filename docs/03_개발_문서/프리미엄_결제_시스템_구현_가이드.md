# 프리미엄 결제 시스템 구현 가이드 (2차 결제)

## 📋 목차
1. [개요](#개요)
2. [유저 플로우](#유저-플로우)
3. [데이터베이스 설계](#데이터베이스-설계)
4. [백엔드 구현](#백엔드-구현)
5. [프론트엔드 구현](#프론트엔드-구현)
6. [테스트 시나리오](#테스트-시나리오)

---

## 개요

### 목적
1차 결제자(제3서 구매자)가 추가 결제를 통해:
1. 수호신 카드에 **자신의 한자 이름**을 새김
2. **전체 결과 PDF**를 받음 (제1서 + 제2서 + 제3서 + 수호신 카드)

### 결제 금액
- **1차 결제**: 3,000원 (테스트: 100원)
- **2차 결제**: TBD (테스트: 100원)

---

## 유저 플로우

```
무료 사용자
  ↓
[1차 결제 (100원)]
  ↓
제3서 + 기본 수호신 카드 ("天命錄" 인장)
  ↓
[수호신 카드 영역에 2개 버튼 표시]
  - 버튼 1: "이름 새기기"
  - 버튼 2: "수호신 카드 다운로드" (기본 버전)
  ↓
"이름 새기기" 클릭 → 한자 이름 입력 모달 오픈
  ↓
한자 이름 입력 (2-4자)
  ↓
[2차 결제 (100원)]
  ↓
① DB에 한자 이름 저장
② isPremium 플래그 TRUE로 업데이트
  ↓
[결과 페이지 리로드]
  ↓
수호신 카드 인장에 한자 이름 표시
  ↓
[버튼 2개 변경]
  - 버튼 1: "수호신 카드 다운로드" (한자 새겨진 버전)
  - 버튼 2: "전체 결과 PDF 다운로드"
```

---

## 데이터베이스 설계

### 1. `payments` 테이블 수정

**새 컬럼 추가:**
```sql
ALTER TABLE payments
ADD COLUMN payment_type ENUM('basic', 'premium') DEFAULT 'basic'
AFTER status;
```

**컬럼 설명:**
- `payment_type`: 결제 타입 구분
  - `basic`: 1차 결제 (제3서 구매)
  - `premium`: 2차 결제 (프리미엄 업그레이드)

---

### 2. `saju_results` 테이블 수정

**새 컬럼 추가:**
```sql
ALTER TABLE saju_results
ADD COLUMN custom_hanja_name VARCHAR(10) DEFAULT NULL
AFTER yongshen_reason,

ADD COLUMN is_premium BOOLEAN DEFAULT FALSE
AFTER custom_hanja_name;
```

**컬럼 설명:**
- `custom_hanja_name`: 사용자가 입력한 한자 이름 (예: "李敏浩")
- `is_premium`: 2차 결제 완료 여부 (TRUE/FALSE)

---

### 3. 마이그레이션 스크립트

**파일 경로:** `backend/scripts/migrate_premium.js`

```javascript
import mysql from 'mysql2/promise';
import dotenv from 'dotenv';

dotenv.config();

async function migratePremium() {
  const connection = await mysql.createConnection({
    host: process.env.DB_HOST,
    user: process.env.DB_USER,
    password: process.env.DB_PASSWORD,
    database: process.env.DB_NAME
  });

  try {
    console.log('🚀 프리미엄 기능 마이그레이션 시작...');

    // 1. payments 테이블 수정
    await connection.execute(`
      ALTER TABLE payments
      ADD COLUMN IF NOT EXISTS payment_type ENUM('basic', 'premium')
      DEFAULT 'basic'
      AFTER status
    `);
    console.log('✅ payments.payment_type 컬럼 추가 완료');

    // 2. saju_results 테이블 수정
    await connection.execute(`
      ALTER TABLE saju_results
      ADD COLUMN IF NOT EXISTS custom_hanja_name VARCHAR(10)
      DEFAULT NULL
      AFTER yongshen_reason
    `);
    console.log('✅ saju_results.custom_hanja_name 컬럼 추가 완료');

    await connection.execute(`
      ALTER TABLE saju_results
      ADD COLUMN IF NOT EXISTS is_premium BOOLEAN
      DEFAULT FALSE
      AFTER custom_hanja_name
    `);
    console.log('✅ saju_results.is_premium 컬럼 추가 완료');

    console.log('🎉 프리미엄 기능 마이그레이션 완료!');

  } catch (error) {
    console.error('❌ 마이그레이션 실패:', error);
    throw error;
  } finally {
    await connection.end();
  }
}

migratePremium().catch(console.error);
```

**실행 방법:**
```bash
cd backend
node scripts/migrate_premium.js
```

---

## 백엔드 구현

### 1. 프리미엄 결제 요청 생성

**파일:** `backend/src/controllers/paymentController.js`

**새 엔드포인트 추가:**

```javascript
/**
 * 2차 결제 (프리미엄 업그레이드) 요청 생성
 * POST /api/payment/premium
 */
export const createPremiumPayment = async (req, res) => {
  try {
    const { accessToken, hanjaName } = req.body;

    // 1. 토큰 검증 및 사주 결과 조회
    const [results] = await db.execute(
      'SELECT * FROM saju_results WHERE access_token = ?',
      [accessToken]
    );

    if (results.length === 0) {
      return res.status(404).json({ error: '사주 결과를 찾을 수 없습니다' });
    }

    const sajuResult = results[0];

    // 2. 1차 결제 완료 확인
    if (!sajuResult.is_paid) {
      return res.status(400).json({ error: '1차 결제가 필요합니다' });
    }

    // 3. 이미 프리미엄 결제했는지 확인
    if (sajuResult.is_premium) {
      return res.status(400).json({
        error: '이미 프리미엄 업그레이드를 완료했습니다'
      });
    }

    // 4. 한자 이름 유효성 검증
    const hanjaRegex = /^[\u4E00-\u9FFF]{2,4}$/;
    if (!hanjaRegex.test(hanjaName)) {
      return res.status(400).json({
        error: '한자 2-4자를 입력해주세요'
      });
    }

    // 5. 한자 이름 임시 저장 (결제 전)
    await db.execute(
      'UPDATE saju_results SET custom_hanja_name = ? WHERE access_token = ?',
      [hanjaName, accessToken]
    );

    // 6. 포트원 결제 요청 데이터 생성
    const paymentData = {
      merchant_uid: `premium_${Date.now()}`,
      amount: 100, // 테스트: 100원 (추후 변경)
      name: '천명록 프리미엄 업그레이드',
      buyer_name: sajuResult.user_name,
      buyer_tel: '010-0000-0000',
      m_redirect_url: `${process.env.CLIENT_URL}/payment/callback`
    };

    res.json({
      success: true,
      paymentData,
      accessToken
    });

  } catch (error) {
    console.error('프리미엄 결제 요청 생성 실패:', error);
    res.status(500).json({ error: '서버 오류가 발생했습니다' });
  }
};
```

---

### 2. 결제 검증 수정

**파일:** `backend/src/controllers/paymentController.js`

**기존 `verifyPayment` 함수 수정:**

```javascript
export const verifyPayment = async (req, res) => {
  try {
    const { imp_uid, merchant_uid } = req.body;

    // payment_type 구분 (merchant_uid로 판단)
    const isPremium = merchant_uid.startsWith('premium_');

    // 포트원 결제 검증 (기존 로직)
    const accessToken = await getPortOneAccessToken();
    const paymentData = await getPaymentData(imp_uid, accessToken);

    // 결제 금액 검증
    const expectedAmount = isPremium ? 100 : 100; // 테스트: 둘 다 100원
    if (paymentData.amount !== expectedAmount) {
      return res.status(400).json({ error: '결제 금액이 일치하지 않습니다' });
    }

    if (isPremium) {
      // 프리미엄 결제: is_premium = TRUE 업데이트
      const [results] = await db.execute(
        'SELECT access_token FROM saju_results WHERE custom_hanja_name IS NOT NULL ORDER BY created_at DESC LIMIT 1'
      );

      if (results.length === 0) {
        return res.status(404).json({ error: '사주 결과를 찾을 수 없습니다' });
      }

      const accessToken = results[0].access_token;

      await db.execute(
        'UPDATE saju_results SET is_premium = TRUE WHERE access_token = ?',
        [accessToken]
      );

      // payments 테이블에 저장
      await db.execute(
        `INSERT INTO payments (merchant_uid, imp_uid, amount, status, payment_type)
         VALUES (?, ?, ?, 'paid', 'premium')`,
        [merchant_uid, imp_uid, paymentData.amount]
      );

      res.json({
        success: true,
        accessToken,
        isPremium: true
      });

    } else {
      // 기본 결제: 기존 로직 유지
      // ... (기존 코드)
    }

  } catch (error) {
    console.error('결제 검증 실패:', error);
    res.status(500).json({ error: '결제 검증 중 오류가 발생했습니다' });
  }
};
```

---

### 3. PDF 생성 엔드포인트

**필요 패키지 설치:**
```bash
cd backend
npm install puppeteer
```

**파일:** `backend/src/controllers/pdfController.js` (신규)

```javascript
import puppeteer from 'puppeteer';
import db from '../config/database.js';

/**
 * PDF 생성 및 다운로드
 * GET /api/pdf/generate/:token
 */
export const generatePDF = async (req, res) => {
  try {
    const { token } = req.params;

    // 1. 프리미엄 사용자 확인
    const [results] = await db.execute(
      `SELECT * FROM saju_results
       WHERE access_token = ? AND is_premium = TRUE`,
      [token]
    );

    if (results.length === 0) {
      return res.status(403).json({
        error: '프리미엄 사용자만 이용 가능합니다'
      });
    }

    const sajuResult = results[0];

    // 2. Puppeteer로 HTML → PDF 변환
    const browser = await puppeteer.launch({
      headless: 'new',
      args: [
        '--no-sandbox',
        '--disable-setuid-sandbox',
        '--disable-dev-shm-usage'
      ]
    });

    const page = await browser.newPage();

    // 결과 페이지 URL 방문 (print 모드)
    const url = `${process.env.CLIENT_URL}/result/${token}?print=true`;
    await page.goto(url, {
      waitUntil: 'networkidle0',
      timeout: 30000
    });

    // PDF 생성
    const pdfBuffer = await page.pdf({
      format: 'A4',
      printBackground: true,
      margin: {
        top: '20mm',
        bottom: '20mm',
        left: '15mm',
        right: '15mm'
      }
    });

    await browser.close();

    // 3. PDF 파일 전송
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="천명록_${sajuResult.user_name}.pdf"`
    );
    res.send(pdfBuffer);

  } catch (error) {
    console.error('PDF 생성 실패:', error);
    res.status(500).json({ error: 'PDF 생성 중 오류가 발생했습니다' });
  }
};
```

**라우터 추가:** `backend/src/routes/pdfRoutes.js` (신규)

```javascript
import express from 'express';
import { generatePDF } from '../controllers/pdfController.js';

const router = express.Router();

router.get('/generate/:token', generatePDF);

export default router;
```

**메인 서버에 라우터 등록:** `backend/src/index.js`

```javascript
import pdfRoutes from './routes/pdfRoutes.js';

app.use('/api/pdf', pdfRoutes);
```

---

### 4. 고화질 수호신 이미지 다운로드

**필요 패키지 설치:**
```bash
cd backend
npm install sharp
```

**파일:** `backend/src/controllers/talismanController.js` (신규)

```javascript
import sharp from 'sharp';
import path from 'path';
import { fileURLToPath } from 'url';
import db from '../config/database.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

/**
 * 고화질 수호신 이미지 다운로드 (한자 각인 포함)
 * GET /api/talisman/download/:token
 */
export const downloadTalismanImage = async (req, res) => {
  try {
    const { token } = req.params;

    // 1. 사주 결과 조회
    const [results] = await db.execute(
      'SELECT * FROM saju_results WHERE access_token = ?',
      [token]
    );

    if (results.length === 0) {
      return res.status(404).json({ error: '사주 결과를 찾을 수 없습니다' });
    }

    const sajuResult = results[0];

    // 2. 1차 결제 확인
    if (!sajuResult.is_paid) {
      return res.status(403).json({ error: '결제가 필요합니다' });
    }

    const talismanType = sajuResult.talisman_name; // 예: "갑자"
    const customName = sajuResult.custom_hanja_name;

    // 3. 원본 이미지 로드
    const imagePath = path.join(
      __dirname,
      '../../public/images/talisman/',
      `${talismanType}.png`
    );

    let image = sharp(imagePath);

    // 4. 프리미엄 사용자일 경우 한자 이름 각인
    if (sajuResult.is_premium && customName) {
      // 한자 이름 포맷팅 (TalismanCard.jsx 로직과 동일)
      let displayName = customName;

      if (customName.length === 2) {
        displayName = customName + '之印';
      } else if (customName.length === 3) {
        displayName = customName + '印';
      }

      // SVG 텍스트 레이어 생성
      const svgText = `
        <svg width="1024" height="1536">
          <style>
            .hanja-text {
              font-family: 'Gungsuh', 'Batang', serif;
              font-size: 60px;
              fill: #b91c1c;
              font-weight: 900;
            }
          </style>
          <text class="hanja-text" x="850" y="1450" text-anchor="middle">
            ${displayName.split('').map((char, i) => {
              const col = i % 2;
              const row = Math.floor(i / 2);
              return `<tspan x="${850 + col * 50}" dy="${row === 0 ? 0 : 60}">${char}</tspan>`;
            }).join('')}
          </text>
        </svg>
      `;

      const textBuffer = Buffer.from(svgText);

      // 이미지 합성
      image = image.composite([
        { input: textBuffer, top: 0, left: 0 }
      ]);
    }

    // 5. PNG로 변환 및 전송
    const buffer = await image.png().toBuffer();

    const fileName = sajuResult.is_premium
      ? `수호신_${sajuResult.user_name}_${customName}.png`
      : `수호신_${sajuResult.user_name}.png`;

    res.setHeader('Content-Type', 'image/png');
    res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
    res.send(buffer);

  } catch (error) {
    console.error('수호신 이미지 생성 실패:', error);
    res.status(500).json({ error: '이미지 생성 중 오류가 발생했습니다' });
  }
};
```

**라우터 추가:** `backend/src/routes/talismanRoutes.js` (신규)

```javascript
import express from 'express';
import { downloadTalismanImage } from '../controllers/talismanController.js';

const router = express.Router();

router.get('/download/:token', downloadTalismanImage);

export default router;
```

**메인 서버에 라우터 등록:** `backend/src/index.js`

```javascript
import talismanRoutes from './routes/talismanRoutes.js';

app.use('/api/talisman', talismanRoutes);
```

---

## 프론트엔드 구현

### 1. 한자 입력 모달 컴포넌트

**파일:** `src/components/HanjaInputModal.jsx` (신규)

```jsx
import React, { useState } from 'react';
import { X } from 'lucide-react';

const HanjaInputModal = ({ isOpen, onClose, onSubmit }) => {
  const [hanjaName, setHanjaName] = useState('');
  const [error, setError] = useState('');

  const handleSubmit = () => {
    // 한자 검증 (2-4자, 한자만)
    const hanjaRegex = /^[\u4E00-\u9FFF]{2,4}$/;

    if (!hanjaRegex.test(hanjaName)) {
      setError('한자 2-4자를 입력해주세요');
      return;
    }

    onSubmit(hanjaName);
  };

  const handleClose = () => {
    setHanjaName('');
    setError('');
    onClose();
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1a1a1c] border border-amber-900/30 rounded-lg p-8 max-w-md w-full">
        {/* 헤더 */}
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-amber-500 font-serif">
            이름 새기기
          </h3>
          <button
            onClick={handleClose}
            className="text-stone-400 hover:text-stone-200 transition-colors"
          >
            <X size={24} />
          </button>
        </div>

        {/* 입력 영역 */}
        <div className="space-y-4">
          <div>
            <label className="text-stone-300 text-sm mb-2 block">
              한자 이름 (2-4자)
            </label>
            <input
              type="text"
              value={hanjaName}
              onChange={(e) => {
                setHanjaName(e.target.value);
                setError('');
              }}
              placeholder="李敏浩"
              className="w-full bg-black/50 border border-stone-700 rounded px-4 py-3
                         text-stone-200 text-lg font-serif text-center tracking-wider
                         focus:border-amber-500 focus:outline-none transition-colors"
              maxLength={4}
            />
            {error && (
              <p className="text-red-500 text-xs mt-1">{error}</p>
            )}
          </div>

          {/* 안내 문구 */}
          <div className="bg-amber-950/20 border border-amber-900/30 rounded p-4 text-xs text-stone-400 space-y-2">
            <p>✨ 수호신 카드에 귀하의 한자 이름이 새겨집니다</p>
            <p>📄 전체 결과 PDF 다운로드가 가능해집니다</p>
          </div>

          {/* 결제 버튼 */}
          <button
            onClick={handleSubmit}
            className="w-full bg-gradient-to-r from-amber-600 to-amber-500
                       hover:from-amber-500 hover:to-amber-400
                       text-white font-bold py-3 rounded-lg shadow-lg
                       transition-all transform hover:scale-105"
          >
            결제하기 (100원)
          </button>
        </div>
      </div>
    </div>
  );
};

export default HanjaInputModal;
```

---

### 2. ResultPage 수정

**파일:** `src/pages/ResultPage.jsx`

**상태 추가:**

```jsx
const [showHanjaModal, setShowHanjaModal] = useState(false);
```

**수호신 카드 아래 버튼 영역 추가:**

```jsx
{/* 수호신 카드 표시 영역 */}
<TalismanCard
  type={sajuResult.talismanName}
  isPurchased={sajuResult.isPaid}
  stampName={sajuResult.isPremium ? sajuResult.customHanjaName : '天命錄'}
/>

{/* 다운로드 버튼 영역 */}
{sajuResult.isPaid && (
  <div className="mt-8 flex flex-col gap-3 w-full max-w-[320px]">
    {!sajuResult.isPremium ? (
      // 1차 결제자용 버튼 (프리미엄 X)
      <>
        <button
          onClick={() => setShowHanjaModal(true)}
          className="w-full bg-gradient-to-r from-amber-600 to-amber-500
                     hover:from-amber-500 hover:to-amber-400
                     text-white font-bold py-3 rounded-lg shadow-lg
                     transition-all transform hover:scale-105
                     flex items-center justify-center gap-2"
        >
          <span className="text-xl">✨</span>
          이름 새기기
        </button>

        <button
          onClick={handleDownloadTalismanCard}
          className="w-full bg-stone-800 hover:bg-stone-700
                     text-stone-200 font-bold py-3 rounded-lg
                     transition-all flex items-center justify-center gap-2"
        >
          <Download size={18} />
          수호신 카드 다운로드
        </button>
      </>
    ) : (
      // 2차 결제자용 버튼 (프리미엄 O)
      <>
        <button
          onClick={handleDownloadTalismanCard}
          className="w-full bg-gradient-to-r from-emerald-600 to-emerald-500
                     hover:from-emerald-500 hover:to-emerald-400
                     text-white font-bold py-3 rounded-lg shadow-lg
                     transition-all transform hover:scale-105
                     flex items-center justify-center gap-2"
        >
          <Download size={18} />
          수호신 카드 다운로드 (한자 새겨짐)
        </button>

        <button
          onClick={handleDownloadPDF}
          className="w-full bg-gradient-to-r from-purple-600 to-purple-500
                     hover:from-purple-500 hover:to-purple-400
                     text-white font-bold py-3 rounded-lg shadow-lg
                     transition-all transform hover:scale-105
                     flex items-center justify-center gap-2"
        >
          <FileDown size={18} />
          전체 결과 PDF 다운로드
        </button>
      </>
    )}
  </div>
)}

{/* 한자 입력 모달 */}
<HanjaInputModal
  isOpen={showHanjaModal}
  onClose={() => setShowHanjaModal(false)}
  onSubmit={handlePremiumPayment}
/>
```

---

### 3. 핸들러 함수

**파일:** `src/pages/ResultPage.jsx`

```jsx
/**
 * 수호신 카드 다운로드 (서버에서 고화질 이미지 생성)
 */
const handleDownloadTalismanCard = async () => {
  try {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/talisman/download/${accessToken}`
    );

    if (!response.ok) {
      throw new Error('다운로드 실패');
    }

    const blob = await response.blob();

    // Blob URL 생성 및 다운로드
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `수호신_${sajuResult.userName}.png`;
    document.body.appendChild(link);
    link.click();

    // 정리
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);

  } catch (error) {
    console.error('수호신 카드 다운로드 실패:', error);
    alert('다운로드 중 오류가 발생했습니다');
  }
};

/**
 * 전체 결과 PDF 다운로드
 */
const handleDownloadPDF = async () => {
  try {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/pdf/generate/${accessToken}`
    );

    if (!response.ok) {
      throw new Error('PDF 생성 실패');
    }

    const blob = await response.blob();

    // Blob URL 생성 및 다운로드
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `천명록_${sajuResult.userName}.pdf`;
    document.body.appendChild(link);
    link.click();

    // 정리
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);

  } catch (error) {
    console.error('PDF 다운로드 실패:', error);
    alert('PDF 다운로드 중 오류가 발생했습니다');
  }
};

/**
 * 프리미엄 결제 처리
 */
const handlePremiumPayment = async (hanjaName) => {
  try {
    // 1. 프리미엄 결제 요청 생성
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/payment/premium`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accessToken, hanjaName })
      }
    );

    const data = await response.json();

    if (!data.success) {
      alert(data.error || '결제 요청 실패');
      return;
    }

    const { paymentData } = data;

    // 2. 포트원 결제 호출
    const IMP = window.IMP;
    IMP.init(process.env.REACT_APP_PORTONE_IMP);

    IMP.request_pay(paymentData, async (rsp) => {
      if (rsp.success) {
        // 3. 결제 검증
        const verifyResponse = await fetch(
          `${process.env.REACT_APP_API_URL}/api/payment/verify`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              imp_uid: rsp.imp_uid,
              merchant_uid: rsp.merchant_uid
            })
          }
        );

        const verifyData = await verifyResponse.json();

        if (verifyData.success) {
          // 4. 성공 시 페이지 새로고침
          alert('프리미엄 업그레이드가 완료되었습니다!');
          window.location.reload();
        } else {
          alert('결제 검증 실패');
        }
      } else {
        alert('결제 실패: ' + rsp.error_msg);
      }
    });

  } catch (error) {
    console.error('프리미엄 결제 오류:', error);
    alert('결제 처리 중 오류가 발생했습니다');
  }
};
```

---

## 테스트 시나리오

### 1. 1차 결제자 → 2차 결제 플로우

1. **무료 사용자로 시작**
   - 사주 정보 입력
   - 1차 결제 (100원) 완료

2. **제3서 + 기본 수호신 카드 확인**
   - 수호신 카드 인장: "天命錄"
   - 버튼 2개 확인:
     - "이름 새기기"
     - "수호신 카드 다운로드"

3. **"이름 새기기" 클릭**
   - 한자 입력 모달 오픈
   - 한자 이름 입력 (예: "李敏浩")
   - "결제하기" 버튼 클릭

4. **2차 결제 (100원) 완료**
   - 포트원 결제 창 확인
   - 결제 완료
   - 페이지 자동 새로고침

5. **프리미엄 기능 확인**
   - 수호신 카드 인장: "李敏浩印"
   - 버튼 2개 변경 확인:
     - "수호신 카드 다운로드 (한자 새겨짐)"
     - "전체 결과 PDF 다운로드"

---

### 2. 다운로드 테스트

1. **수호신 카드 다운로드 (1차 결제자)**
   - 기본 버전 다운로드
   - 인장: "天命錄"

2. **수호신 카드 다운로드 (2차 결제자)**
   - 한자 새겨진 버전 다운로드
   - 인장: "李敏浩印" (2자), "李敏浩之印" (3자)

3. **PDF 다운로드 (2차 결제자만)**
   - 전체 결과 PDF 다운로드
   - 내용 확인: 제1서 + 제2서 + 제3서 + 수호신 카드

---

### 3. 에러 케이스 테스트

1. **한자 검증 실패**
   - 한글 입력 → 에러 메시지
   - 1자 입력 → 에러 메시지
   - 5자 이상 입력 → 에러 메시지

2. **중복 결제 시도**
   - 이미 프리미엄인 사용자가 다시 결제 시도
   - → "이미 프리미엄 업그레이드를 완료했습니다" 에러

3. **1차 결제 없이 2차 결제 시도**
   - → "1차 결제가 필요합니다" 에러

4. **비로그인 사용자 PDF 다운로드 시도**
   - → "프리미엄 사용자만 이용 가능합니다" 에러

---

## 주요 기술 개념

### 1. Sharp vs html2canvas

| 구분 | Sharp (서버) | html2canvas (클라이언트) |
|------|-------------|-------------------------|
| **실행 위치** | 백엔드 (Node.js) | 프론트엔드 (브라우저) |
| **화질** | 고화질 (원본 품질 유지) | 중간 (화면 캡처 수준) |
| **폰트** | 서버 폰트 사용 | 브라우저 폰트 사용 |
| **보안** | 높음 (서버에서 처리) | 낮음 (클라이언트 조작 가능) |
| **용도** | 최종 다운로드 | 실시간 미리보기 |

### 2. Puppeteer PDF 생성

- **원리**: 브라우저를 헤드리스 모드로 실행하여 HTML → PDF 변환
- **장점**: CSS, 폰트, 이미지 모두 정확하게 렌더링
- **주의사항**: 메모리 사용량이 높으므로 `browser.close()` 필수

### 3. Blob 다운로드 패턴

```javascript
const blob = await response.blob();
const url = window.URL.createObjectURL(blob);
const link = document.createElement('a');
link.href = url;
link.download = 'filename.png';
link.click();
window.URL.revokeObjectURL(url);
```

---

## 구현 순서

1. ✅ 백엔드 이미지 복사 (`public/images/talisman` → `backend/public/images/talisman`)
2. ✅ DB 마이그레이션 (`scripts/migrate_premium.js`)
3. ✅ 백엔드 - 프리미엄 결제 엔드포인트 (`createPremiumPayment`)
4. ✅ 백엔드 - 결제 검증 수정 (`verifyPayment`)
5. ✅ 백엔드 - PDF 생성 엔드포인트 (`pdfController.js`) - 기존 구현 수정
6. ✅ 백엔드 - 수호신 이미지 다운로드 (`talismanController.js`)
7. ✅ 프론트엔드 - 한자 입력 모달 (`HanjaInputModal.jsx`)
8. ✅ 프론트엔드 - ResultPage 버튼 영역 추가
9. ⬜ 테스트 및 디버깅

---

## 주의사항

### 백엔드

1. **Puppeteer 메모리 관리**
   - PDF 생성 후 반드시 `browser.close()` 호출
   - 타임아웃 설정 필수 (`timeout: 30000`)

2. **Sharp 이미지 합성**
   - 한자 폰트가 서버에 설치되어 있어야 함
   - Windows: `Batang`, `Gungsuh` 폰트
   - Linux: `fonts-nanum` 패키지 설치

3. **결제 검증**
   - `merchant_uid`로 basic/premium 구분 철저히
   - 금액 검증 필수

### 프론트엔드

1. **한자 검증**
   - 정규식: `/^[\u4E00-\u9FFF]{2,4}$/`
   - 프론트와 백엔드 모두에서 검증 필수

2. **Blob 메모리 관리**
   - `createObjectURL()` 후 반드시 `revokeObjectURL()` 호출
   - 메모리 누수 방지

3. **포트원 결제**
   - `merchant_uid`에 `premium_` 접두사 필수
   - 결제 검증 후 페이지 새로고침

---

## FAQ

### Q1. PDF 용량이 너무 큽니다. 어떻게 최적화하나요?

**A1.** Puppeteer PDF 옵션 조정:

```javascript
const pdfBuffer = await page.pdf({
  format: 'A4',
  printBackground: true,
  preferCSSPageSize: true,
  displayHeaderFooter: false,
  // 이미지 품질 조정
  quality: 80, // JPEG 품질 (1-100)
});
```

### Q2. 한자 폰트가 깨집니다. 어떻게 해결하나요?

**A2.** 서버에 한자 폰트 설치:

```bash
# Ubuntu/Debian
sudo apt-get install fonts-nanum fonts-nanum-extra

# CentOS/RHEL
sudo yum install google-noto-cjk-fonts
```

### Q3. Puppeteer가 서버에서 실행되지 않습니다.

**A3.** 의존성 패키지 설치:

```bash
# Ubuntu/Debian
sudo apt-get install -y \
  chromium-browser \
  ca-certificates \
  fonts-liberation \
  libappindicator3-1 \
  libasound2 \
  libatk-bridge2.0-0 \
  libatk1.0-0 \
  libc6 \
  libcairo2 \
  libcups2 \
  libdbus-1-3 \
  libexpat1 \
  libfontconfig1 \
  libgbm1 \
  libgcc1 \
  libglib2.0-0 \
  libgtk-3-0 \
  libnspr4 \
  libnss3 \
  libpango-1.0-0 \
  libpangocairo-1.0-0 \
  libstdc++6 \
  libx11-6 \
  libx11-xcb1 \
  libxcb1 \
  libxcomposite1 \
  libxcursor1 \
  libxdamage1 \
  libxext6 \
  libxfixes3 \
  libxi6 \
  libxrandr2 \
  libxrender1 \
  libxss1 \
  libxtst6 \
  lsb-release \
  wget \
  xdg-utils
```

---

## 참고 문서

- [동적 이미지 각인 및 다운로드 구현 가이드](./동적_이미지_각인_및_다운로드_구현_가이드.md)
- [포트원 결제 연동 가이드](https://portone.gitbook.io/docs/)
- [Puppeteer 공식 문서](https://pptr.dev/)
- [Sharp 공식 문서](https://sharp.pixelplumbing.com/)
